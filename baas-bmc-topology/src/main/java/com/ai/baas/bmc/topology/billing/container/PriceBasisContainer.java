...
public class PriceBasisContainer {
	...
	private static String addNewData(String tenant_id,String unit_type){
		...
		//List<Map<String, String>> results = CacheProxy.getInstance().doQuery(priceBasisTable, params);
		List<Map<String, String>> results = dshmClient.list(priceBasisTable).where(params).executeQuery(CacheProxy.getInstance().getCacheClient());
		String addupSubject = "";
