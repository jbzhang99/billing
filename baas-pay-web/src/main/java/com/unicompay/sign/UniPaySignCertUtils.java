...
import com.unicompay.util.Constants;


public class UniPaySignCertUtils {
	 ...
	 * 
	 * @param reqMap
	 *            签名数据Map (注意：null 或 空值 或 一些特殊字符串不参与验签)
	 * @param signType
	 *            签名类型 SHA256_RSA，SM2_SM3
	 * @return
	 */
	 ...
	 * @date 2016年12月28日
	 * @param reqMap
	 *            请求map参数 (注意：null 或 空值 或 一些特殊字符串不参与验签)
	 * @param signType
	 *            签名类型 SHA256_RSA，SM2_SM3
	 * @param cert
	 *            联通支付公司证书base64
	 ...
	 * 
	 */
	public static boolean merVerify(Map<String, String> reqMap,
			String signType, String signMsg, String cert) {
		if (!Constants.SIGN_TYPE_DICT.contains(signType)) {
			logger.info("不支持此签名类型:" + signType);
			return false;
		} else {
			try {
				return rsaOrSm2Verify(UniPaySignCertUtils.getSignSourMsg(reqMap),signMsg, signType, cert);
			} catch (Exception e) {
				e.printStackTrace();
			}
			return false;
	...
	private static Map<String, String> filterNoParams(Map<String, String> params) {
		...
		for (Map.Entry<String, String> entry : params.entrySet()) {
			...
			if (Constants.EXCEPT_SIGNMSG.equalsIgnoreCase(key)
					|| Constants.EXCEPT_CERT.equalsIgnoreCase(key)
	 ...
	 * 
	 */
	private static boolean rsaOrSm2Verify(String originalData, String signMsg,
			String algorithm, String cert) throws Exception {
		/**
		 * 0x00000103：SHA1_RSA签名算法 0x00000105：SHA256_RSA签名算法
		...
		if (Constants.SIGNTYPE_RSA_SHA256.equalsIgnoreCase(algorithm)) {
			verifyWithCert = CAUtility.verifyWithCert(0x00000105, certbytes,
					indata, signMsgbytes);
		} else if (Constants.SIGNTYPE_SM2_SM3.equalsIgnoreCase(algorithm)) {
			verifyWithCert = CAUtility.EccVerifySignByCert(0x00000104,
					certbytes, indata, signMsgbytes);
		}
	 ...
	 * 
	 */
	private static String rsaOrSm2Sign(String originalData, String algorithm)
			throws Exception {
		byte[] indata = originalData.getBytes(Constants.CHARSET_UTF8);
		byte[] signBytes = null;
		signBytes = CAUtility.RsaSignByP12(TypeConstant.CA_SHA256WITHRSA,
				indata);
