...
import com.ai.opt.sys.service.business.interfaces.IIndustryBusiSV;
...
public class TenantValidateSVImpl implements ITenantValidateSV {
	...
	public void checkTenantName(String tenantName) throws BusinessException {
		if (StringUtil.isBlank(tenantName)) {
			throw new BusinessException(SystemExceptCode.ErrorCode.PARAM_NULL_ERROR, "企业名称（tenantName）不能为空");
		}
		if (RegexUtils.checkHasSpecialChar(tenantName)) {
			throw new BusinessException(SystemExceptCode.ErrorCode.PARAM_VALUE_ERROR, "企业名称（tenantName）不能含有特殊字符");
		}
		if (tenantName.contains("\u0020")) {
			throw new BusinessException(SystemExceptCode.ErrorCode.PARAM_VALUE_ERROR, "企业名称（tenantName）不能包含空格");
		}
		...
		if (tenantNameSize < TENANTNAME_MINSIZE || tenantNameSize > TENANTNAME_MAXSIZE) {
			throw new BusinessException(SystemExceptCode.ErrorCode.PARAM_VALUE_ERROR, "企业名称（tenantName）长度为" + TENANTNAME_MINSIZE + "~" + TENANTNAME_MAXSIZE + "位字符");
		}
	}
	...
	public void checkIndustryCode(String industryCode) throws BusinessException {
		if (StringUtil.isBlank(industryCode)) {
            throw new BusinessException(SystemExceptCode.ErrorCode.PARAM_NULL_ERROR,
                    "企业类型（industryCode）不能为空");
        }
		int length = StringUtil.getByteLength(industryCode);
		if(length>INDUSTRYCODE_MAXSIZE){
			throw new BusinessException(SystemExceptCode.ErrorCode.PARAM_VALUE_ERROR,
	...
	public void checkAccountId(Long accountId) throws BusinessException {
		if (accountId == null) {
			throw new BusinessException(SystemExceptCode.ErrorCode.PARAM_NULL_ERROR, "账号ID（accountId）不能为空");
		}
	}
	...
	public void checkTenantId(String tenantId) throws BusinessException {
		if (StringUtil.isBlank(tenantId)) {
			throw new BusinessException(SystemExceptCode.ErrorCode.PARAM_NULL_ERROR, "租户ID（tenantId）不能为空");
		}
		// 参数规则检查
		int length = StringUtil.getByteLength(tenantId);
		if (length > TENANTID_MAXSIZE) {
			throw new BusinessException(SystemExceptCode.ErrorCode.PARAM_VALUE_ERROR, "租户ID（tenantId）长度不能超过" + TENANTID_MAXSIZE);
		}
	}
