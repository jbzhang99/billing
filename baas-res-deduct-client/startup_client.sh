...
DEDUCT_HOME=$HOME/test      #定义程序的根目录
export DEDUCT_HOME

#把根目录下面所有的jar包都加入到CP环境变量中
for file in ${DEDUCT_HOME}/3rd-libs/*.jar;
do CP=${CP}:$file;
done

for file in ${DEDUCT_HOME}/core-libs/*.jar;
do CP=${CP}:$file;
done
...
#启动java虚拟机和程序
${JAVA_HOME}/bin/java ${MEM_ARGS}   ${JAVA_OPTIONS} com.ai.runner.center.bmc.deduct.consumer.AuthProduct  > ${DEDUCT_HOME}/logs/client.log  2>&1 &
