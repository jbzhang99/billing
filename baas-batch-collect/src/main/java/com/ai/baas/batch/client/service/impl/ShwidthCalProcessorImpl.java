 ...
 * 
 * @ClassName: ShwidthCalProcessorImpl.java
 * @Description: 共享带宽
 *
 * @version: v1.0.0
 * @author: hanzf
 * @date: 2017年4月25日 上午11:08:03
 *
 * Modification History:
 * Date             Author          Version            Description
 *---------------------------------------------------------*
 * 2017年4月25日     hanzf           v1.0.0               创建
 */
public class ShwidthCalProcessorImpl implements ICalProcessor{

	
	private static final Logger LOG = LoggerFactory
			.getLogger(NatCalProcessorImpl.class);

	@Override
	public void buildCalAdapt(String response, String serviceId)
			throws Exception {
		String sharewidth = getUrl.getInstance().getShare();
		if (!("".equals(sharewidth))) {
			response = sharewidth;
			LOG.debug("the sharewidth  " + sharewidth);
		}
		...
		for (int i = 0; i < jsonarray.length; i++) {
			...
			String userId = null;
			if (results == null) {
				System.out.println("nat instance id " + instanceId
						+ "  not get user_id");
			} else {
				...
				timeString.append(
						lastDayUtil.messageHead(userId,
								...
								ClientConstants.ASHARE_SYSTEM)).append(
						ClientConstants.HEAD_SPLIT);
				StringBuilder timeMessage = new StringBuilder();
				timeMessage.append(ClientConstants.SOURCE_SHARE)
