 ...
 * 
 * @ClassName: YupCalProcessorImpl.java
 * @Description: 新增云盘的报文转换
 * 
 * @version: v1.0.0
 * @author: hanzf
...
public class YupCalProcessorImpl implements ICalProcessor {
	private static final Logger LOG = LoggerFactory
			.getLogger(YupCalProcessorImpl.class);

	@Override
	public void buildCalAdapt(String response, String serviceId)
			throws Exception {
		String yup = getUrl.getInstance().getYUP();
		if (!("".equals(yup))) {
			response = yup;
			LOG.debug("the eip yup  " + yup);
		}
		...
		for (int i = 0; i < jsonarray.length; i++) {
			...
			String userId = null;
			if (results == null) {
				System.out.println("yup instance id " + instanceId
						+ "  not get user_id");
			} else {
				...
				timeString.append(
						lastDayUtil.messageHead(userId,
								...
								ClientConstants.AYUP_SYSTEM)).append(
						ClientConstants.HEAD_SPLIT);
				StringBuilder timeMessage = new StringBuilder();
				timeMessage.append(ClientConstants.SOURCE_YUP)
