 ...
 * 
 * @ClassName: NatCalProcessorImpl.java
 * @Description: nat网关
 *
 * @version: v1.0.0
 * @author: hanzf
 * @date: 2017年4月25日 上午11:01:27
 *
 ...
 */
public class NatCalProcessorImpl implements ICalProcessor{
	...
	public void buildCalAdapt(String response, String serviceId)
			throws Exception {
		String snat = getUrl.getInstance().getNat();
		if (!("".equals(snat))) {
			response = snat;
			LOG.debug("the nat  " + snat);
		}
			...
			if (results == null) {
				...
				timeString.append(
						lastDayUtil.messageHead(userId,
								...
								ClientConstants.ANAT_SYSTEM)).append(
						ClientConstants.HEAD_SPLIT);
				StringBuilder timeMessage = new StringBuilder();
				timeMessage.append(ClientConstants.SOURCE_NAT)
