 ...
 * 
 * @ClassName: EipCalProcessorImpl.java
 * @Description: 新增EIP的报文转换
 *
 * @version: v1.0.0
 * @author: hanzf
 * @date: 2017年2月27日 下午4:53:58
 *
 * Modification History:
 * Date             Author          Version            Description
 *---------------------------------------------------------*
 * 2017年2月27日     hanzf           v1.0.0               创建
 */

public class EipCalProcessorImpl  implements ICalProcessor{
	private static final Logger LOG=LoggerFactory.getLogger(EipCalProcessorImpl.class);
	@Override
	public void buildCalAdapt(String response,String serviceId) throws Exception {
		String eip=getUrl.getInstance().getEIP();
		if(!("".equals(eip))){
			response=eip;
			LOG.debug("the eip is  "+eip);
		}
		...
		for(int i=0;i<jsonarray.length;i++){
			...
			String userId=null;
			if(results==null){
				System.out.println("eip instance id "+instanceId +"  not get user_id");	
			}
			else{
				...
				timeString.append(lastDayUtil.messageHead(userId, ClientConstants.SOURCE_TIME,ClientConstants.AEIP_SYSTEM)).append(ClientConstants.HEAD_SPLIT);
				StringBuilder timeMessage= new StringBuilder();
				timeMessage.append(ClientConstants.SOURCE_EIP).append(ClientConstants.MESSAGE_SPLIT).append(userId).append(ClientConstants.MESSAGE_SPLIT).append(instanceId)
				...
				if("0".equals(netOut))
					...
					streamString.append(lastDayUtil.messageHead(userId, ClientConstants.SOURCE_STREAM,ClientConstants.AEIP_SYSTEM)).append(ClientConstants.HEAD_SPLIT);
					StringBuilder streamMessage= new StringBuilder();
					streamMessage.append(ClientConstants.SOURCE_EIP).append(ClientConstants.MESSAGE_SPLIT).append(userId).append(ClientConstants.MESSAGE_SPLIT).append(instanceId)
					...
					System.out.println("the senttile stream is "+sentStream.toString());
				...
				String lastTime=lastDayUtil.difHour(startTime, endTime);
				String start=lastDayUtil.norTime(startTime);
				String networkOut = jsonarray[i].getString(ClientConstants.NET_OUT);
				//开始进行消息头的拼接
				StringBuilder timeString=new StringBuilder();
				timeString.append(lastDayUtil.messageHead(userId, ClientConstants.SOURCE_TIME,ClientConstants.AEIP_SYSTEM)).append(ClientConstants.HEAD_SPLIT);
				StringBuilder timeMessage= new StringBuilder();
				timeMessage.append(ClientConstants.SOURCE_EIP).append(ClientConstants.MESSAGE_SPLIT).append(userId).append(ClientConstants.MESSAGE_SPLIT).append(instanceId)
				.append(ClientConstants.MESSAGE_SPLIT).append(regionId).append(ClientConstants.MESSAGE_SPLIT).append(networkOut).append(ClientConstants.MESSAGE_SPLIT).append(start).append(ClientConstants.MESSAGE_SPLIT).append("STREAM");
				timeString.append(timeMessage.toString());
				//开始进行时间消息的发送
				StringBuilder sentTime=new StringBuilder();
				sentTime.append(ClientConstants.RTMHEAD).append(timeString.toString()).append(ClientConstants.RTMEND);
				LOG.debug("the rtmmsg:" + sentTime);
				HttpClientUtil.send(rtmUrl, sentTime.toString());*/
			}
		}
	}
