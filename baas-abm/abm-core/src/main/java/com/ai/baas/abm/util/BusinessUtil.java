...
public final class BusinessUtil {
    ...
    public static void checkRequestHeader(RequestHeader requestHeader) throws BusinessException {
        if (requestHeader == null) {
            throw new BusinessException(ExceptCodeConstants.PARAM_IS_NULL, "[请求报文头为空]");
        }
        if (StringUtil.isBlank(requestHeader.getApplyChlId())) {
            throw new BusinessException(ExceptCodeConstants.PARAM_IS_NULL, "[报文头渠道ID为空]");
        }
        if (StringUtil.isBlank(requestHeader.getSystemId())) {
            throw new BusinessException(ExceptCodeConstants.PARAM_IS_NULL, "[报文头系统ID为空]");
        }
        if (requestHeader.getOperId() == null || requestHeader.getOperId() == 0) {
            throw new BusinessException(ExceptCodeConstants.PARAM_IS_NULL, "[报文头操作员ID为空]");
        }
    }
    ...
    public static void checkBaseInfo(BaseInfo baseInfo) throws BusinessException {
        if (null == baseInfo) {
            throw new BusinessException(ExceptCodeConstants.PARAM_IS_NULL, "[请求报文]为空");
        }
        if (StringUtil.isBlank(baseInfo.getTenantId())) {
            throw new BusinessException(ExceptCodeConstants.PARAM_IS_NULL, "[租户ID]为空");
        }
    }    
}
