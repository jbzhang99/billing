...
public class PackResultImpl implements IPackResult {
    ...
    @Override
    public ResourceDeposit chResourceDeposit(FunResBook book,String accountPeriod) {
        ResourceDeposit rd = new ResourceDeposit();
        rd.setAccountPeriod(accountPeriod);
        
        rd.setSystemId(book.getSYSTEM_ID());
        rd.setTenantId(book.getTENANT_ID());
        rd.setExternalId(DateUtil.getDateString(DateUtil.yyyyMMddHHmmssSSS)+RandomUtils.nextInt());
        // 暂时写死
        rd.setOwnerType(0);
