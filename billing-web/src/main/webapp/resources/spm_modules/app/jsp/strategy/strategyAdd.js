define('app/jsp/strategy/strategyAdd', function (require, exports, module) {
    'use strict';
    var $=require('jquery'),
    ...
    //定义页面组件类
    var StrategyAddPager = Widget.extend({
        //属性，使用时由类的构造函数传入
        attrs: {
        	varTemp:$("#varTemp"),
        ...
        //重写父类
        setup: function () {
            StrategyAddPager.superclass.setup.call(this);
            this._initFunction();
        },
...
//        	 this._bindFactorValueChange();
        ...
        _bindFactorValueDisabled:function(){
        	$(".detailList").find("div.detailTemp").each(function(index){
        ...
        _bindFactorValueChange:function(){
        	...
        	$(".first-factors").find(".factor").each(function(index){
        		...
        		$(this).find("input[name='factorValue']").bind("input propertychange",function(){
        			var value = $(this).val();
        			$(".detailList").find("div.detailTemp").each(function(){
					...
					$('select.var-type').each(function(index){
						...
						$.each(
								...
								function(index, item) {
									// 循环获取数据
									$(_this)
											.append('<option unid="'+json[index].id+'" uncode="'+json[index].paramCode+'" unname="'+json[index].paramName+'">'+"变量类型-"+json[index].paramName+'</option>');
								});
						$(_this)
								.append("<label id='accesstype_error'></label>");
					});
...
//						.append('<option unid="-1" uncode="" unname="">请选择变量单位</option>');
    ...
    module.exports = StrategyAddPager;
});
