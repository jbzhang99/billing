define("arale-calendar/1.1.2/index-month", ["jquery","moment/2.9.0/moment-debug","position/1.1.0/index-debug","arale-widget/1.2.0/widget-debug","arale-base/1.2.0/base-debug","arale-class/1.2.0/class-debug","arale-events/1.2.0/events-debug","arale-iframe-shim/1.1.0/index-debug","import-style/1.0.0/index-debug"], function(require, exports, module){
...
var Calendar = BaseCalendar.extend({
  ...
  setup: function() {
    ...
    this.months.on('select', function(value, el) {
      ...
      self.years.select(focus);
      if (el) {
        self.trigger('selectMonth', value);
        var dataValue = focus._d;
    ...
    this.years.on('select', function(value, el) {
      ...
      if (el && el.data('role') === 'year') {
        self.renderContainer('months', focus);
...
function template(model, options) {
  ...
  html += '<tfoot><tr><th class="today index-reset" style="text-align:center;height:30px;" colspan="7">清除</th></tr></tfoot>';
  html += '</table>';
  return html;
}
