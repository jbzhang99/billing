define('paging/0.0.1/paging', function (require, exports, module) {
    'use strict';
var Paging = {
    render: function(){},
    createData: function(){},
...
Paging.mustache = require('mustache/2.0.0/mustache')
...
+ '{{#_hasPaging}}'
+ '    <div class="ui-paging">'
+ '        <{{#_prevPage}}a{{/_prevPage}}{{^_prevPage}}span{{/_prevPage}} class="ui-paging-prev" onclick="refresh({{_prevPage}})" >'
+ '            上一页'
+ '        </{{#_prevPage}}a{{/_prevPage}}{{^_prevPage}}span{{/_prevPage}}>'
+ '        {{^_isFirstPage}}'
+ '        <a onclick="refresh(1)" class="ui-paging-item">1</a>'
+ '        {{/_isFirstPage}}'
+ '        {{#_hasBeforePages}}'
+ '        <span class="ui-paging-ellipsis">...</span>'
+ '        {{/_hasBeforePages}}'
+ '        {{#_beforePages}}'
+ '        <a class="ui-paging-item" onclick="refresh({{.}})">{{.}}</a>'
+ '        {{/_beforePages}}'
+ '        <a onclick="refresh({{_currentPage}})"  class="ui-paging-item ui-paging-current">{{_currentPage}}</a>'
+ '        {{#_afterPages}}'
+ '        <a class="ui-paging-item" onclick="refresh({{.}})">{{.}}</a>'
+ '        {{/_afterPages}}'
+ '        {{#_hasAfterPages}}'
...
+ '        {{/_hasAfterPages}}'
+ '        {{^_isLastPage}}'
+ '        <a  onclick="refresh({{_pageCount}})"  class="ui-paging-item">{{_pageCount}}</a>'
+ '        {{/_isLastPage}}'
+ '        <{{#_nextPage}}a{{/_nextPage}}{{^_nextPage}}span{{/_nextPage}} class="ui-paging-next" onclick="refresh({{_nextPage}})" link>'
+ '            下一页'
...
+ '        <a class="ui-paging-info ui-paging-goto" onclick="refresh({{_currentPage}})" >跳转</a>'
+ '    </div>'
+ '{{/_hasPaging}}'
