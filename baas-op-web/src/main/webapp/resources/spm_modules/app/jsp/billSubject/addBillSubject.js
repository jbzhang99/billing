define('app/jsp/billSubject/addBillSubject', function (require, exports, module) {
    'use strict';
    var $=require('jquery'),
    ...
    //定义页面组件类
    var AddBillSubjectPager = Widget.extend({
        //属性，使用时由类的构造函数传入
        attrs: {
        },
        //事件代理
        ...
        //重写父类
        setup: function () {
            AddBillSubjectPager.superclass.setup.call(this);
            this._initPage();
            //this._bindValidator();
        },
        _initPage: function(){
        	...
        	if(_subjectId!=undefined&&_subjectId!=""&&_subjectId!="0"){
        		setBreadCrumb("计费配置管理","修改账单科目");
        	}
        	else{
        		setBreadCrumb("计费配置管理","新增账单科目");
        	}
    	},
        cancelAdd:function(){
           var url = _base + "/billSubject/list";
...
//   		        alertFlag: true
        ...
        submitAddTable:function(){
            ...
            if(!param.test(subjectName)){
                ...
                ajaxController.ajax({
                    type: "post",
                    dataType : "json",
                    url: _base+"/billSubject/addBillSubject",
                    processing: true,
                    message: "正在加载，请等待...",
                    data:{
    ...
    module.exports = AddBillSubjectPager;
});
