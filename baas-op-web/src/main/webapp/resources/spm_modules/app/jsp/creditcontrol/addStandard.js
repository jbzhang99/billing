define('app/jsp/creditcontrol/addStandard', function (require, exports, module) {
    'use strict';
    var $=require('jquery'),
    ...
    //定义页面组件类
    var AddStandardPager = Widget.extend({
    	//属性，使用时由类的构造函数传入
    	attrs: {
    	},
    	//事件代理
    	...
    	//重写父类
    	setup: function () {
    		AddStandardPager.superclass.setup.call(this);
    		this._initPage();
    	},
    	 _initPage: function(){
    		//左侧菜单选中样式
    	...
    	_checkRuleName:function(){
    		...
    		if(ruleName == "" || ruleName == null || ruleName == undefined){
    			...
    			return false;
    		}else if(ruleName.replace(/[^\x00-\xff]/g,"aaa").length>40){
    	...
    	_addStanderd:function(){
			...
			ajaxController.ajax({
				...
				dataType: 'json',
				url :_base+"/creditControl/standard/insertStandard",
				processing: true,
				message : "正在处理中，请稍候...",
    ...
    module.exports = AddStandardPager
});
