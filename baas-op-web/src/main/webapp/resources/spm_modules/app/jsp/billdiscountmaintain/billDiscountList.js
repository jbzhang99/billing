define('app/jsp/billdiscountmaintain/billDiscountList', function (require, exports, module) {
    'use strict';
    var $=require('jquery'),
    ...
    var BillDiscountPager = Widget.extend({
    	...
    	_initPage: function(){
    		$("#mnu_discount_mng").addClass("current");//左侧菜单选中样式
    		setBreadCrumb("产品资费管理","优惠策略管理");//导航
      	},
    	//日期
    	_bindCalendar: function(){
    		new Calendar({
    			trigger: '#effectDate_be',
		...
		_checkDate: function(){
			var sTime = $("#effectDate_be").val();
			var dTime = $("#expireDate_be").val();
			if(sTime!="" && dTime!=""){
			      //js判断日期
    	...
    	_searchBtnClick: function(){
    		...
    		$("#pagination-ul").runnerPagination({
	 			url: _base + "/billDiscountMaintain/getList",
	 			method: "POST",
	 			dataType: "json",
	 			processing: true,
    		...
    		$(".delFlag").bind("click", function(){
				...
				Dialog({
                    ...
                    ok:function(){
                    	$.ajax({
                    		type : "POST",
                    		url : _base + "/billDiscountMaintain/delete",
                    		data: {
