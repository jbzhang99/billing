...
import com.ai.baas.bmc.util.ExpressionUtil;
...
public class PricemakingBusiSVImpl implements IPricemakingBusiSV {
    ...
    public List<PriceInfo> queryPricemaking(PriceElementInfo request) {
        ...
        for (OrderTypeInfo orderTypeInfo : orderTypeList) {
            ...
            for (Entry<String, List<Map<String, String>>> entry : map.entrySet()) {
                ...
                for (Map<String, String> m : entry.getValue()) {
                    ...
                    for (ElementInfo elementInfo : elementInfos) {
                        if (factorName.toLowerCase().equals(elementInfo.getName().toLowerCase())
                                && factorValue.toLowerCase().equals(
                                        elementInfo.getValue().toLowerCase())) {
                            isFactorMatch = true;
                            break;
                        }
                    }
                    if (!CollectionUtil.isEmpty(extInfos)) {
                        for (ExtInfo extInfo : extInfos) {
                            if (factorName.toLowerCase().equals(extInfo.getExtName().toLowerCase())
                                    && factorValue.toLowerCase().equals(
                                            extInfo.getExtValue().toLowerCase())) {
                                isFactorMatch = true;
                                break;
                            }
                        }
            ...
            CpPricemakingRuleCriteria cpPricemakingRuleCriteria = new CpPricemakingRuleCriteria();
            CpPricemakingRuleCriteria.Criteria criteria= cpPricemakingRuleCriteria.createCriteria();
            criteria.andTenantIdEqualTo(tenantId)
                    .andPriceProductTypeEqualTo(priceType)
                    .andPriceProductIdEqualTo(priceProductId)
                    .andActiveTimeLessThan(sysdate)
                    .andInactiveTimeGreaterThanOrEqualTo(sysdate);
            if (!StringUtil.isBlank(innerPriceType)) {
                criteria.andPriceTypeEqualTo(innerPriceType);
            }
            List<CpPricemakingRule> cpPricemakingRules = cpPricemakingRuleMapper
                    .selectByExample(cpPricemakingRuleCriteria);
            ...
            // 计算费用值
            String price = "";
            if (BmcConstants.CpPricemakingRule.RuleCode.CONST.equals(cpPricemakingRule.getRuleCode())) {
                // 常量
                price = cpPricemakingRule.getRuleExpresion();
            } else if (BmcConstants.CpPricemakingRule.RuleCode.EXPR.equals(cpPricemakingRule.getRuleCode())) {
                HashMap<String, String> variables = new HashMap<String, String>();
                for (ElementInfo elementInfo : elementInfos) {
                    variables.put(elementInfo.getName().toLowerCase(), elementInfo.getValue());
                }
                if (!CollectionUtil.isEmpty(extInfos)) {
                    for (ExtInfo extInfo : extInfos) {
                        variables.put(extInfo.getExtName().toLowerCase(), extInfo.getExtValue());
                    }
                }
                if (!StringUtil.isBlank(cpPricemakingRule.getExtInfo())) {
                    ...
                    if (extConst != null) {
                        for (Entry<String, String> entry : extConst.entrySet()) {
                            variables.put(entry.getKey().toLowerCase(), entry.getValue());
                        }
                    }
                }
                // 表达式
                String expression = cpPricemakingRule.getRuleExpresion().toLowerCase();
            ...
            } else if (BmcConstants.CpPricemakingRule.RuleCode.STEP.equals(cpPricemakingRule.getRuleCode())) {
                // 阶梯
                ...
                String baseValue = "";
                for (ElementInfo elementInfo : elementInfos) {
                    if (elementInfo.getName().toLowerCase()
                            .equals(cpPricemakingRule.getExtInfo().toLowerCase())) {
                ...
                if (StringUtil.isBlank(baseValue) && !CollectionUtil.isEmpty(extInfos)) {
                    for (ExtInfo extInfo : extInfos) {
                        if (extInfo.getExtName().toLowerCase()
                                .equals(cpPricemakingRule.getExtInfo().toLowerCase())) {
                ...
                if (StringUtil.isBlank(baseValue)) {
                    throw new BusinessException("未获取到阶梯基准值， 请检查参数中是否存在key：" + cpPricemakingRule.getExtInfo());
                }
                ...
                for (StepInfo stepInfo : stepInfos) {
                    ...
                    String valueTmp = stepInfo.getValue();
                    if (isExpression(stepInfo.getValue())) {
                        Map<String, String> variables = new HashMap<String, String>();
                        variables.put(cpPricemakingRule.getExtInfo(), baseValue);
                        valueTmp = ExpressionUtil.getExpressionResult(stepInfo.getValue(), variables);
                    } 
            ...
            } else if (BmcConstants.CpPricemakingRule.RuleCode.BRACKET.equals(cpPricemakingRule.getRuleCode())) {
                // 分档
                if (StringUtil.isBlank(cpPricemakingRule.getRuleExpresion())) {
                    throw new BusinessException("定价产品ID[" + priceProductId + "]价格规则为空");
                }
                String baseValue = "";
                for (ElementInfo elementInfo : elementInfos) {
                    if (elementInfo.getName().toLowerCase()
                ...
                if (StringUtil.isBlank(baseValue)) {
                    throw new BusinessException("未获取到分档基准值， 请检查参数中是否存在key：" + cpPricemakingRule.getExtInfo());
                }

                double baseValueDouble = Double.parseDouble(baseValue);
                double priceDouble = 0;
                List<StepInfo> stepInfos = JSON.parseArray(cpPricemakingRule.getRuleExpresion(), StepInfo.class);
                for (StepInfo stepInfo : stepInfos) {
                    double start = Double.parseDouble(stepInfo.getStart());
                    double end = Double.parseDouble(stepInfo.getEnd());
                    String valueTmp = stepInfo.getValue();
            ...
            //修改 保留三位小数
            feeInfo.setPrice(new BigDecimal(Double.parseDouble(price) / 1000).setScale(3, BigDecimal.ROUND_HALF_UP).toString());
            feeInfo.setPriceUnit(StringUtil.isBlank(cpPricemakingRule.getPriceUnit()) ? "" : cpPricemakingRule.getPriceUnit());
            feeInfo.setPriceUnitName(cpPricemakingRule.getPriceUnitName());
