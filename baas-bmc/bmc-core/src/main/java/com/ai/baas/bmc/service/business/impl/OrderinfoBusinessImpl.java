...
import com.ai.baas.bmc.api.orderinfo.params.*;
...
public class OrderinfoBusinessImpl implements IOrderinfoBusiSV {
    ...
    public void writeData(OrderInfoParams request) {
        ...
        if (StringUtil.isBlank(subsId)) {
            ...
            if (CollectionUtil.isEmpty(result)) {
                ...
                custName = result.get(0).get(BmcCacheConstant.Dshm.FieldName.CUST_NAME);
            }
            // 查询账户信息
        ...
        if (request.getProductList() != null) {
            for (Product p : request.getProductList()) {
                writeBlSubsComm(p, tenantId, custId, subsId, request.getSublist(), acctId);
            }
        }
        if (needCreateOweInfo) {
            OweInfoCreateRequest oweInfoCreateRequest = new OweInfoCreateRequest();
            oweInfoCreateRequest.setTenantId(request.getTenantId());
    ...
    @Override
    public void initInfo(InitCustomerRelatedInfoReq request) {
        boolean needCreateOweInfo = false;
        // 解析参数
        String tenantId = request.getTenantId();
        String extCustId = request.getExtCustId();
        ...
        if (StringUtil.isBlank(subsId)) {
            ...
            params.put(BmcCacheConstant.Dshm.FieldName.EXT_CUST_ID, extCustId);
            params.put(BmcCacheConstant.Dshm.FieldName.TENANT_ID, tenantId);
            List<Map<String, String>> result = DshmUtil.getClient().list(BmcCacheConstant.Dshm.TableName.BL_CUSTINFO)
                    .where(params).executeQuery(DshmUtil.getCacheClient());
    ...
    private void writeBlSubsComm(Product product, String tenantId, String custId, String subsId,
            List<ExtInfo> extInfos, String acctId) {
        ...
        for (int i = 0; i < product.getProductNumber(); i++) {
            ...
            if (!CollectionUtil.isEmpty(extInfos)) {
                for (ExtInfo extInfo : extInfos) {
                    if ("apportion_list".equals(extInfo.getExtType())) {
                        if (!StringUtil.isBlank(extInfo.getExtValue())) {
                            @SuppressWarnings("rawtypes")
                            List<Map> listInfos = JSON.parseArray(extInfo.getExtValue(), Map.class);
                            for (Map<String, String> map : listInfos) {
                                map.put("acct_id", acctId);
                            }
                            extInfo.setExtValue(JSON.toJSONString(listInfos));
                        }
                    }
                    BlSubscommExt blSubscommExt = new BlSubscommExt();
                    blSubscommExt.setExtId(BmcSeqUtil.getSubscommExtId());
                    ...
                    blSubscommExt.setExtValue(extInfo.getExtValue());
                    blSubscommExtMapper.insertSelective(blSubscommExt);
                    // 刷新用户扩展信息表的内存表
                    iBlSubscommExtAtomSV.addDshmData(blSubscommExt);
                }
