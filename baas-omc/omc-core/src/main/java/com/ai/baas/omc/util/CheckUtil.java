...
public class CheckUtil {
    ...
    public static String check(Object o, String name, boolean canBeNull, int lenth,
        ...
        if (StringUtils.isEmpty(o)) {
            if (canBeNull) {
                return ExceptCodeConstant.SUCCESS;
            }
            return ExceptCodeConstant.PARAM_IS_NULL +":" +name + "不能为空";
        }

        if (o.toString().length() > lenth) {
            return ExceptCodeConstant.OVER_LENTH + ":" +name + "不能超过" + lenth + "位";
        }
        ...
        if (StringUtils.isEmpty(enums) || enums.length <= 0) {
	        ...
	        String result = ExceptCodeConstant.UNKNOWN + ":" +name + "取值范围";
	        for (String e : enums) {
