...
public class ScoutActSmsExt extends ScoutActSms {
    ...
    public ScoutActSmsExt() {}

    public ScoutActSmsExt(OmcObj owner, InformationProcessor info, ConfigContainer cfg,
     ...
     * @throws
     */
    public int warning(String ownertype, String oid, OmcObj owner, InformationProcessor info,
            ConfigContainer cfg, RealTimeBalance balance, JsonObject data) throws OmcException {
        setDefault(owner, info, cfg, balance, data);

        // 根据本地信控状态判断是否需要预警，对于提醒到用户的需要判断用户资料的状态，提醒的账户和客户的不判断资料
        if ((ownertype.equals(OwnerType.SERV))) {
            OmcScoutStatus scoutStatus = scoutStatusService.selectStatus(this.getOmcobj()
                    .getTenantid(), this.getOmcobj().getBusinesscode(), oid);
            if (scoutStatus != null) {
        ...
        // 获取当前预警状态
        OmcUrgeStatus omcUrgeStatus = urgeStatusService.selectUrgeStatus(this.getOmcobj()
                .getTenantid(), this.getOmcobj().getBusinesscode(), this.getSectionRule()
                .getSectiontype(), ownertype, oid);
