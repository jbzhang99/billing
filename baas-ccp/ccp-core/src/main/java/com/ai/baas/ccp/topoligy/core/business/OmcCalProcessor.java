...
import com.ai.baas.ccp.topoligy.core.constant.rule.*;
...
public final class OmcCalProcessor extends BaseProcess {
    ...
    @Autowired
    private transient BalanceCalProcessor balanceCalProcessor;

    @Autowired
    private transient CreditCalProcess creditCalProcess;

    public OmcCalProcessor(ConfigContainer cfg, OmcObj obj, JsonObject data) throws OmcException {
        super(cfg, obj, data);
    }

    public void process(ConfigContainer cfg, OmcObj obj, JsonObject jsonObject) throws OmcException {
        setDefault(cfg, obj, jsonObject);
        ...
        // 信控规则字符串转换为信控规则对象
        List<SectionRule> sectionRules = OmcUtils.toSectionRules(cfg, rules);
        // 准备资料 获取三户信息
        info.process(cfg, obj, jsonObject);
        this.setInfo(info);

        // 余额计算
        balanceCalProcessor.process(this.getConfig(), info, jsonObject);
        RealTimeBalance realTimeBalance = balanceCalProcessor.getBalance();

        // 获取信用度
        creditCalProcess.process(this.getConfig(), info, jsonObject);
    ...
    private List<SectionRule> thresholdMatch(RealTimeBalance balance,
            List<SectionRule> sectionRules, String policyid) throws OmcException {
        // 欠费天数
    ...
    private List<SectionRule> matchBalance(RealTimeBalance balance, List<SectionRule> sectionRules,
            String policyid) throws OmcException {
        List<SectionRule> sRules = new ArrayList<SectionRule>();
        ConfigContainer cfg = this.getConfig();
        String matchbalance = cfg.getCfgPara(OmcCalKey.OMC_CFG_MATCH_BALANCE, this.getOmcobj()
                .getTenantid(), policyid, "");
        String stopaddcredit = cfg.getCfgPara(OmcCalKey.OMC_CFG_STOP_ADDCREDITLINE, this
                .getOmcobj().getTenantid(), policyid, "");
        String startaddcredit = cfg.getCfgPara(OmcCalKey.OMC_CFG_START_ADDCREDITLINE, this
                .getOmcobj().getTenantid(), policyid, "");
        String warnaddcredit = cfg.getCfgPara(OmcCalKey.OMC_CFG_WARN_ADDCREDITLINE, this
                .getOmcobj().getTenantid(), policyid, "");
    ...
    private List<SectionRule> matchCharge(RealTimeBalance balance, List<SectionRule> sectionRules,
        ...
        List<SectionRule> sRules = new ArrayList<SectionRule>();
        ConfigContainer cfg = this.getConfig();
        String matchcharge = cfg.getCfgPara(OmcCalKey.OMC_CFG_MATCH_CHARGE, this.getOmcobj()
                .getTenantid(), policyid, "");
    ...
    private List<SectionRule> matchOwners(RealTimeBalance balance, List<SectionRule> sectionRules,
        ...
        List<SectionRule> sRules = new ArrayList<SectionRule>();
        ConfigContainer cfg = this.getConfig();
        String matchowners = cfg.getCfgPara(OmcCalKey.OMC_CFG_MATCH_OWNERS, this.getOmcobj()
                .getTenantid(), policyid, "");
    ...
    private List<SectionRule> matchCreditLevel(Customer customer, List<SectionRule> sectionRules,
        ...
        ConfigContainer cfg = this.getConfig();
        // 是否参照客户级别
        String matchcreditlevel = cfg.getCfgPara(OmcCalKey.OMC_CFG_MATCH_CREDITLEVEL, this
                .getOmcobj().getTenantid(), policyid, "");
